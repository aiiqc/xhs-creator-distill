...
所有输入最终映射到以下字段；不支持的额外字段不得进入分析输入：
...
| `title` | 标题 |
| `content` | 正文；输入兼容别名 `body`，但两者不能同时出现，规范输出统一为 `content` |
...
- 输入必须是单个 UTF-8 CSV 文件，表头至少包含 `title`，并且恰好包含 `content` 或兼容别名 `body` 其中一个；其余使用规范字段。
- 一行对应一个来源记录。字段缺失、表头冲突、编码或 CSV 结构错误按 schema/格式错误处理，不猜测列含义。

### JSON

- 输入必须是 UTF-8 JSON，且顶层为记录数组，或只把顶层 `items` 数组作为记录集合。
- 每个记录必须是对象并含 `title`，并且恰好含 `content` 或兼容别名 `body` 其中一个；其余使用规范名称。
...
固定键包括 `schema_version`、`status`、`input_mode`、`input_format`、`material_scope`、`canonical_fields`、`counts`、`limits`、`selection_policy`、`evidence_mapping`、`hold_reasons`、`output_files`。状态只允许：
...
| `2` | CLI、输入格式、schema 或资源限制错误 | `HOLD`；修正输入，不猜字段或静默截断 |
...
对格式/schema 错误使用退出码 `2`；对可审计但不能继续蒸馏的资料使用 `HOLD` 与退出码 `3`；对输出或文件系统冲突使用退出码 `4`。不要为了取得 `READY` 而联网补资料、解压、读取相邻路径、跟随链接、改写原文件或预填结论。

## 交给 Skill 前的核对

- `manifest.json` 状态为 `READY`，键与五个制品名称均符合契约；
